'''
Observer Design Pattern - This is behavioral design pattern.
'''
from abc import ABC, abstractmethod
class User(ABC):  # Observer class
	'''
...
class BlogWriter:
	'''
	BlogWriter class is useful to blog writer to add new article and manage subscribers as well
	'''
...
if __name__ == "__main__":   
	...
	suraj = User('Suraj')
	nirabh = User('Nirabh')
	blog_writer.subscribe(suraj)
	blog_writer.subscribe(nirabh)
	blog_writer.add_article('Article 1')
	blog_writer.unsubscribe(nirabh)
	blog_writer.add_article('Article2')
